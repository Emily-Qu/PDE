...
class PhysicsInformedNN():
    ...
    def set_training_data(self, x0, u0, v0, tb, Xf, X_prev=None, u_prev=None, v_prev=None):
        # 确保所有输入数据都是PyTorch张量
        ...
        # self.x_lb = torch.tensor(0*tb + self.lb[0], dtype=torch.float32).to(device)
        # self.x_ub = torch.tensor(0*tb + self.ub[0], dtype=torch.float32).to(device)
        ...
        if X_prev is not None:
            ...
            self.v_prev = torch.tensor(v_prev, dtype=torch.float32).to(device)
        else:
            self.X_prev = None
    
    def backward(self):
        # 初始条件损失
        output0 = self.dnn(torch.cat([self.x0, self.t0], dim=1))
        u0_pred = output0[:, 0:1]
        v0_pred = output0[:, 1:2]
        ...
        # 边界条件损失
        output_lb = self.dnn(torch.cat([self.x_lb, self.tb], dim=1))
        output_ub = self.dnn(torch.cat([self.x_ub, self.tb], dim=1))
    ...
    def net_f(self, x, t):
        # 使用 torch.cat 而不是 torch.concat
        output_uv = self.dnn(torch.cat([x, t], dim=1))
        u = output_uv[:, 0:1]
        v = output_uv[:, 1:2]
        ...
        lambda_reg = 1e-9
        reg_term = lambda_reg * (
            torch.mean(torch.square(u_x)) +
            torch.mean(torch.square(u_xx)) +
            torch.mean(torch.square(u_t)) +
            torch.mean(torch.square(v_x)) +
            torch.mean(torch.square(v_xx)) +
            torch.mean(torch.square(v_t))
        ) 
    ...
    def predict(self, X_star):
        ...
        with torch.no_grad():
            ...
            u_star = op_star[:, 0:1].cpu().numpy()
            v_star = op_star[:, 1:2].cpu().numpy()
...
if __name__ == "__main__":
    ...
    full_v_pred = np.zeros_like(v_star)
    full_h_pred = np.zeros_like(h_star)
    ...
    for win_idx in range(num_windows):
        ...
        # 模型文件名
        model_name = f"model_win_{win_idx+1}.pth"
        ...
        if TRAIN_MODE:
            ...
            if win_idx == 0:
                ...
                # 第一个窗口没有前一个模型
                X_prev, u_prev, v_prev = None, None, None
            else:
                # 后续窗口使用前一个窗口在t=t_min时刻的解作为初始条件
                X_init = np.hstack((x, np.ones_like(x)*t_min))
                u_pred, v_pred = prev_model.predict(X_init)
                ...
                if overlap_max > overlap_min:
                    ...
                    u_prev, v_prev = prev_model.predict(X_prev)
            ...
            logfilename = f"window_{win_idx+1}.txt"
            
            model.set_training_data(x0, u0, v0, tb, Xf, X_prev, u_prev, v_prev)
        ...
        full_v_pred[window_mask] = v_pred[window_mask]
        full_h_pred[window_mask] = h_pred[window_mask]
    ...
    if not np.all(predicted_mask):
        ...
        u_missing, v_missing = prev_model.predict(X_star[missing_mask])
        h_missing = np.sqrt(u_missing**2 + v_missing**2)
        
        full_u_pred[missing_mask] = u_missing
        full_v_pred[missing_mask] = v_missing
        full_h_pred[missing_mask] = h_missing
    
    # 计算整体误差
    error_u = np.linalg.norm(u_star - full_u_pred, 2) / np.linalg.norm(u_star, 2)
    error_v = np.linalg.norm(v_star - full_v_pred, 2) / np.linalg.norm(v_star, 2)
    error_h = np.linalg.norm(h_star - full_h_pred, 2) / np.linalg.norm(h_star, 2)
    print('Overall Error u: %e' % (error_u))
    print('Overall Error v: %e' % (error_v))
    print('Overall Error h: %e' % (error_h))
    
    # 可视化拼接后的结果
    Full_U_pred = griddata(X_star, full_u_pred.flatten(), (X, T), method='cubic')
    Full_V_pred = griddata(X_star, full_v_pred.flatten(), (X, T), method='cubic')
    Full_H_pred = griddata(X_star, full_h_pred.flatten(), (X, T), method='cubic')
    Full_Error = np.abs(Exact_h - Full_H_pred)
...
###第一行第一列：预测热图
    ...
    im1 = ax1.imshow(Full_H_pred.T, cmap='viridis', 
...
# 第一行第二列：误差热图
    ...
    im2 = ax2.imshow(Full_Error.T, cmap='viridis', 
                extent=[t.min(), t.max(), x.min(), x.max()], 
                origin='lower', aspect='auto')
...
# 第二行：切片图
    ...
    ax4.plot(x, Exact_h[50,:], 'b-', linewidth=2, label='Exact')
    ax4.plot(x, Full_H_pred[50,:], 'r--', linewidth=2, label='Prediction')
    ax4.set_xlabel('$x$')
    ax4.set_ylabel('$|u(t,x)|$')
    ax4.set_title('$t = -0.59$', fontsize=12)
    ax4.set_xlim([-5.1,5.1])
    ax4.set_ylim([-0.01,4.1])
    ax4.grid(True, linestyle='--', alpha=0.5)
    ax4.set_aspect((5 - (-5)) / (4 - 0), adjustable='box')

    ax5 = fig.add_subplot(gs_row2[1])
    ax5.plot(x, Exact_h[100,:], 'b-', linewidth=2, label='Exact')       
    ax5.plot(x, Full_H_pred[100,:], 'r--', linewidth=2, label='Prediction')
    ax5.set_xlabel('$x$')
    ax5.set_ylabel('$|u(t,x)|$')
    ax5.set_title('$t = 0.79$', fontsize=12)
    ax5.set_xlim([-5.1,5.1])
    ax5.set_ylim([-0.01,4.1])
    ax5.grid(True, linestyle='--', alpha=0.5)
    ax5.set_aspect((5 - (-5)) / (4 - 0), adjustable='box')
    ax5.legend(loc='upper center', bbox_to_anchor=(0.5, -0.2), ncol=2, frameon=False)

    ax6 = fig.add_subplot(gs_row2[2])
    ax6.plot(x, Exact_h[150,:], 'b-', linewidth=2, label='Exact')       
    ax6.plot(x, Full_H_pred[150,:], 'r--', linewidth=2, label='Prediction')
    ax6.set_xlabel('$x$')
    ax6.set_ylabel('$|u(t,x)|$')
    ax6.set_title('$t = 0.98$', fontsize=12)
    ax6.set_xlim([-5.1,5.1])
    ax6.set_ylim([-0.01,4.1])
    ax6.grid(True, linestyle='--', alpha=0.5)
    ax6.set_aspect((5 - (-5)) / (4 - 0), adjustable='box')
    ...
    ######根据模式保存不同名称的图片
    if TRAIN_MODE:
        plt.savefig(os.path.join(current_dir, 'png', 'trained_prediction.png'))
    else:
        plt.savefig(os.path.join(current_dir, 'png', 'loaded_prediction.png'))
    plt.show()
...
#     # 滑动窗口参数 - 创建50%重叠
#     total_time = ub[1] - lb[1]  # 总时间长度
#     num_windows = 3             # 窗口数量
#     window_size = total_time * 0.5  # 每个窗口的时间长度
#     step_size = window_size * 0.5   # 窗口滑动步长 (50%重叠)
...
#     for win_idx in range(num_windows):
#         print(f"\n===== Training Window {win_idx+1}/{num_windows} =====")
...
#     im3 = ax3.imshow(Exact_h.T, cmap='viridis', 
#                     extent=[t.min(), t.max(), x.min(), x.max()], 
#                     origin='lower', aspect='auto')
#     plt.colorbar(im3, ax=ax3)
#     ax3.set_xlabel('$t$')
#     ax3.set_ylabel('$x$')
#     ax3.set_title('Exact $|u(t,x)|$')
...
#         t_min = lb[1] + i * step_size
#         t_max = min(t_min + window_size, ub[1])
#         ax4.axvline(x=t_min, color=colors[i], linestyle='--', alpha=0.7)
#         ax4.axvline(x=t_max, color=colors[i], linestyle='--', alpha=0.7)
#         ax4.axvspan(t_min, t_max, alpha=0.1, color=colors[i], label=f'Window {i+1}')
#     ax4.set_xlabel('Time')
#     ax4.set_ylabel('Window')
#     ax4.set_title('Time Windows with Overlap')
#     ax4.legend()
    
#     plt.tight_layout()
#     plt.savefig(os.path.join(current_dir, 'png', 'combined_prediction_with_overlap_loss.png'))
#     plt.show()