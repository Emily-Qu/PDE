...
# 在代码开头添加训练模式标志
TRAIN_MODE = False  # 设置为True进行训练，False则加载已训练模型进行测试
...
if __name__ == "__main__":
    ...
    for win_idx in range(num_windows):
        ...
        # 模型文件名
        model_name = f"sw(0.1+0)t-2_{win_idx+1}.pth"
        ...
        if TRAIN_MODE:
            ...
            logfilename = f"(0.1+0)t-2_{win_idx+1}.txt"
...
# 第二行：切片图
    ...
    ######根据模式保存不同名称的图片
    if TRAIN_MODE:
        plt.savefig(os.path.join(current_dir, 'png', 'sw-(0.1+0)t-2-train.png'))
    else:
        plt.savefig(os.path.join(current_dir, 'png', 'sw-(0.1+0)t-2-loaded.png'))
    plt.show()
