...
class PhysicsInformedNN():
    ...
    def set_training_data(self, x0, u0, v0, tb, Xf):
        ...
        self.t0 = torch.full_like(self.x0, -1).to(device)
...
if __name__ == "__main__":
    ...
    # 设置beta值
    beta = 0.0  # 可自由修改为任意 float

# 空间和时间网格
    x = np.linspace(-2, 2, 256)[:, None]  # shape: (256, 1)
    t = np.linspace(-1, 1, 201)[:, None]  # shape: (101, 1)

# 构造网格
    X, T = np.meshgrid(x, t)  # shape: (101, 256)
    X = X.T  # 转置成 [256, 101]
    T = T.T

# 计算解析表达式
    theta = -2 * beta * X + (4 * beta**2 + 2) * T
    F1 = -4 * X**2 - 32 * beta * T * X - 16 * (4 * beta**2 + 1) * T**2 + 3
    G1 = 16 * T
    D1 = 4 * X**2 + 32 * beta * T * X + 16 * (4 * beta**2 + 1) * T**2 + 1

# 复解 U[1](x,t)
    U = np.exp(1j * theta) * (F1 + 1j * G1) / D1
...
# 边界（用于后续训练）
    ...
    print("Rogue wave data generated for beta =", beta)
...
# ---- 左图：U_pred ----
    ...
    h0 = ax0.imshow(H_pred.T, interpolation='nearest', cmap='YlGnBu', 
...
# ---- 右图：误差图 Error = |Exact - Pred| ----
    ...
    h1 = ax1.imshow(Error.T, interpolation='nearest', cmap='rainbow',
                extent=[t.min(), t.max(), x.min(), x.max()], 
                origin='lower', aspect='auto')#cmap='YlGnBu',cmap='rainbow'
    ...
    ax.set_xlabel('$x$')
    ax.set_ylabel('$u(t,x)$')
    ax.set_title('$t = -5.0$', fontsize = 10)
    ax.axis('square')
    ax.set_xlim([-2.1,2.1])
    ax.set_ylim([-0.01,4.1])   
    ax.set_aspect( ( 2- (-2)) / (4 - 0), adjustable='box' )
    ...
    fig.savefig(os.path.join(current_dir, 'png', 'testpinn-0.3+0.3.png'))
    plt.show()
