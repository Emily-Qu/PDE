...
class PhysicsInformedNN():
    ...
    def set_training_data(self, x0, u0, v0, tb, Xf, X_prev=None, u_prev=None, v_prev=None):
        ...
        self.t0 = torch.full_like(self.x0, -2.0).to(device)
    ...
    def train(self, nIter, logfilename=None, init_iter=0, LBFGS_max_iter=50000):
        self.dnn.train()
        self.iter = init_iter
        self.optimizer_Adam = torch.optim.AdamW(self.dnn.parameters(),weight_decay=1e-2)
...
if __name__ == "__main__":
    ...
    for win_idx in range(num_windows):
        ...
        # 模型文件名
        model_name = f"adamWsw(0.1+0)t-2_{win_idx+1}.pth"
        ...
        if TRAIN_MODE:
            ...
            logfilename = f"adamW(0.1+0)t-2_{win_idx+1}.txt"
...
# 第二行：切片图
    ...
    ######根据模式保存不同名称的图片
    if TRAIN_MODE:
        plt.savefig(os.path.join(current_dir, 'png', 'adamWsw(0.1+0)t-2-train.png'))
    else:
        plt.savefig(os.path.join(current_dir, 'png', 'adamWsw-(0.1+0)t-2-loaded.png'))
    plt.show()
