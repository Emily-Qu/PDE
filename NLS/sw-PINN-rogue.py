...
if __name__ == "__main__":
    # 加载数据
    data = scipy.io.loadmat(os.path.join(current_dir, 'data', 'NLS_rogue.mat'))
    ...
    for win_idx in range(num_windows):
        ...
        # 模型文件名
        model_name = f"win_{win_idx+1}.pth"
...
# 第二行：切片图
    ...
    ax4.set_aspect((2 - (-2)) / (4 - 0), adjustable='box')
    ...
    ax5.set_aspect((2 - (-2)) / (4 - 0), adjustable='box')
    ...
    ax6.set_aspect((2 - (-2)) / (4 - 0), adjustable='box')
    ...
    ######根据模式保存不同名称的图片
    if TRAIN_MODE:
        plt.savefig(os.path.join(current_dir, 'png', 'sw-NLS-rogue-train.png'))
    else:
        plt.savefig(os.path.join(current_dir, 'png', 'sw-NLS-rogue-loaded.png'))
    plt.show()
