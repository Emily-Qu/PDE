...
if __name__ == "__main__":
    ...
    logfilename = "log-PINN53[-0.5,1][-15,20].txt"
    model.set_training_data(x0, u0, tb, Xf)
    model.train(500, logfilename)
    # model_name = "PINN1soliton.pth"
    model_name = "PINN53[-0.5,1][-15,20].pth"
...
# ---- 右图：误差图 Error = |Exact - Pred| ----
    ...
    ax.axis('square')
    ax.set_xlim([-15.1,20.1])
    ax.set_ylim([-0.01,0.5]) 
    ax.set_aspect( (20 - (-15)) / (0.5 - 0), adjustable='box' )
    # ax.set_xlim([-15.1,15.1])
    # ax.set_ylim([-0.3,0.3]) 
    # ax.set_aspect( (15 - (-15)) / (0.3 - (-0.3)), adjustable='box' )
    ...
    fig.savefig(os.path.join(current_dir, 'png', 'PI 53[-0.5,1][-15,20].png'))
    plt.show()
