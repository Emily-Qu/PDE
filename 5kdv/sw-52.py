...
if __name__ == "__main__":
    ...
    for win_idx in range(num_windows):
        ...
        # 模型文件名
        model_name = f"sw52[-0.5,1][-10,15]_{win_idx+1}.pth"
        ...
        if TRAIN_MODE:
            ...
            logfilename = f"sw52[-0.5,1][-10,15]_{win_idx+1}.txt"
...
# 第二行：切片图
    ...
    ######根据模式保存不同名称的图片
    if TRAIN_MODE:
        plt.savefig(os.path.join(current_dir, 'png', 'sw-52[-0.5,1][-10,15]-train.png'))
    else:
        plt.savefig(os.path.join(current_dir, 'png', 'sw-52[-0.5,1][-10,15]-loaded.png'))
    plt.show()
