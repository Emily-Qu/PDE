...
if __name__ == "__main__":
    # 加载数据
    data = scipy.io.loadmat(os.path.join(current_dir, 'data', '53[-0.5,1][-15,20].mat'))
    ...
    for win_idx in range(num_windows):
        ...
        # 模型文件名
        model_name = f"sw53adamW[-0.5,1][-15,20]_{win_idx+1}.pth"
        ...
        if TRAIN_MODE:
            ...
            logfilename = f"sw53adamW[-0.5,1][-15,20]_{win_idx+1}.txt"
...
# 第二行：切片图
    ...
    ax4.set_xlim([-15.1,20.1])
    ax4.set_ylim([-0.01,0.5])
    ax4.grid(True, linestyle='--', alpha=0.5)
    ax4.set_aspect((20 - (-15)) / (0.5 - 0), adjustable='box')
    ...
    ax5.set_xlim([-15.1,20.1])
    ax5.set_ylim([-0.01,0.5])
    ax5.grid(True, linestyle='--', alpha=0.5)
    ax5.set_aspect((20 - (-15)) / (0.5 - 0), adjustable='box')
    ...
    ax6.set_xlim([-15.1,20.1])
    ax6.set_ylim([-0.01,0.5])
    ax6.grid(True, linestyle='--', alpha=0.5)
    ax6.set_aspect((20 - (-15)) / (0.5 - 0), adjustable='box')
    ...
    ######根据模式保存不同名称的图片
    if TRAIN_MODE:
        plt.savefig(os.path.join(current_dir, 'png', 'adamWsw-53[-0.5,1][-15,20]-train.png'))
    else:
        plt.savefig(os.path.join(current_dir, 'png', 'adamWsw-53[-0.5,1][-15,20]-loaded.png'))
    plt.show()
