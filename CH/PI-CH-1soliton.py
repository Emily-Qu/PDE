...
class PhysicsInformedNN():
    ...
    def net_f(self, x, t):  # 在backward中调用
        ...
        return u_t - u_xxt + 3*u*u_x -2*u_xx*u_x  -u_xxx*u
...
if __name__ == "__main__":
    ...
    data = scipy.io.loadmat(os.path.join(current_dir, 'data', 'CH-201*512.mat'))
...
# ---- 右图：误差图 Error = |Exact - Pred| ----
    ...
    ax.set_ylim([-0.01,5]) 
    ax.set_aspect( (15 - (-15)) / (5 - 0), adjustable='box' )
    # ax.set_xlim([-15.1,15.1])
    # ax.set_ylim([-0.3,0.3]) 
    ...
    fig.savefig(os.path.join(current_dir, 'png', 'PI CH1.png'))
    plt.show()
