...
#     torch.cuda.manual_seed_all(1234)  # 多GPU的情况
...
#         # 假设输入变量都是 numpy array
#         self.lb = torch.tensor(lb, dtype=torch.float32)
#         self.ub = torch.tensor(ub, dtype=torch.float32)
#         self.nb = tb.shape[0]  # tb 是 (N, 1)

#         # 初始条件
#         self.x0 = torch.tensor(X0[:, 0:1], dtype=torch.float32, requires_grad=True)
#         self.t0 = torch.tensor(X0[:, 1:2], dtype=torch.float32, requires_grad=True)
#         self.u0 = torch.tensor(u0, dtype=torch.float32)

#         # 边界条件（左边界）
#         self.x_lb = torch.tensor(X_lb[:, 0:1], dtype=torch.float32, requires_grad=True)
#         self.t_lb = torch.tensor(X_lb[:, 1:2], dtype=torch.float32, requires_grad=True)

#         # 边界条件（右边界）
#         self.x_ub = torch.tensor(X_ub[:, 0:1], dtype=torch.float32, requires_grad=True)
#         self.t_ub = torch.tensor(X_ub[:, 1:2], dtype=torch.float32, requires_grad=True)
...
#         # 用于测试或评估的点（真值）
#         self.x_star = torch.tensor(X_star[:, 0:1], dtype=torch.float32)
#         self.t_star = torch.tensor(X_star[:, 1:2], dtype=torch.float32)
...
#             raise
    ...
    data = scipy.io.loadmat('./AC_R_1.mat') #加载数据
    
    t = data['tt'].flatten()[:,None] #加载时间
    #x = data['x'].flatten()[:,None]
    Exact = data['uu'] #加载函数解
    Exact_u = np.real(Exact) #加载函数解的实部
    Nx = 50 #x取点
    x = np.linspace(-1,1,Nx) #加载x
    x = x.flatten()[:,None] #展开x
    dx = x[2] - x[1] #？
    
    x0 = x #网格上的x的点
    #print(x0.shape)
    u0 = (x**2)*np.cos(np.pi*x) #初始时刻的解形式
    #u0 = Exact_u[:,0]
    u0 = u0.reshape(x.shape) #初始时刻在x网格上的解的值
