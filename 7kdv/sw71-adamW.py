...
class PhysicsInformedNN():
    ...
    def net_f(self, x, t):  # 在backward中调用
        ...
        return  u_t + 140*u*u*u*u_x + 70*u_x*u_x*u_x + 280*u*u_x*u_xx + 70*u*u*u_xxx +70*u_xx*u_xxx + 42*u_x*u_xxxx + 14*u*u_xxxxx + u_xxxxxxx
...
if __name__ == "__main__":
    # 加载数据
    data = scipy.io.loadmat(os.path.join(current_dir, 'data', '71[-2,2]201*512.mat'))
    ...
    for win_idx in range(num_windows):
        ...
        # 模型文件名
        model_name = f"sw71adamW_{win_idx+1}.pth"
        ...
        if TRAIN_MODE:
            ...
            logfilename = f"sw71adamW_{win_idx+1}.txt"
...
# 第二行：切片图
    ...
    ax4.set_aspect((10 - (-10)) / (0.6 - 0), adjustable='box')
    ...
    ax5.set_ylim([-0.01,0.6])
    ax5.grid(True, linestyle='--', alpha=0.5)
    ax5.set_aspect((10 - (-10)) / (0.6 - 0), adjustable='box')
    ...
    ax6.set_ylim([-0.01,0.6])
    ax6.grid(True, linestyle='--', alpha=0.5)
    ax6.set_aspect((10 - (-10)) / (0.6 - 0), adjustable='box')
    ...
    ######根据模式保存不同名称的图片
    if TRAIN_MODE:
        plt.savefig(os.path.join(current_dir, 'png', 'adamWsw-71-train.png'))
    else:
        plt.savefig(os.path.join(current_dir, 'png', 'adamWsw-71-loaded.png'))
    plt.show()
